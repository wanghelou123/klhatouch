...
using namespace std;
		...
		serial::serial(int _baudRate, int _dataBit, char _parity, int _stopBit)
:baud_i(_baudRate), dataBit(_dataBit), parity(_parity),stopBit(_stopBit)
{
#ifdef DEBUG
		cout <<"Serial parameters: " << endl;
		std::cout <<baud_i<<"\t"
				<<dataBit<<"\t"
				<<parity<<"\t"
				<<stopBit<<"\t"
...
{
		if((fd=open(SERIAL_DEV_NAME, O_RDWR | O_NOCTTY | O_NDELAY)) == -1){
