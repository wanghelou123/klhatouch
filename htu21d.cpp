...
#define I2CBus             "/dev/i2c-5"      //i2c-gpio

// Returns a file id for the port/bus
int htu21d::i2c_Open(char *I2CBusName)
{
	int fd;
...
void htu21d::i2c_commonsenderror(int fd,uint8_t DeviceAddress)
{
	printf("\n");
...
int htu21d::htu21df_crcdata(const uint8_t *Data, uint16_t *ConvertedValue)
{
	uint32_t dataandcrc;
...
int htu21d::htu21df_ReadUserReg(int fd, int *retValue)
{
...
int htu21d::htu21df_WriteUserReg(int fd,int NewValue)
{
	int UserRegVal;
...
int htu21d::htu21df_init(int fd)
{
	int rc;
	int UserReg;
	struct i2c_rdwr_ioctl_data messagebuffer;
...
int htu21d::htu21df_getValue(int fd, float *Value, const uint8_t YourBiddingMaster )
{
...
float htu21d::htu21df_compensatedRH(float RH,float TempCell)
{
	return ( RH+((25-TempCell)*-0.15) ) ;
}

float htu21d::htu21df_pptamb(float Temp)
{
...
float htu21d::htu21df_DewPoint(float Temp, float Hum)
{
...
htu21d::htu21d()
{
	HTU21DF_ResetDelay = 15;
	HTU21DF_MinDelay   = 11; //Should be 16-RetryDelay (16 or so for hum 50 for temp)
	HTU21DF_RetryDelay = 5; 

	htu21df_ACo=8.1332;
	htu21df_BCo=1762.39;
	htu21df_CCo=235.66;
	fd = i2c_Open(I2CBus); //program will terminate within function if bus not present.
	float myTemp,myHum;
